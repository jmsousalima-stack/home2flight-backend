...
function getOperationalImpactMinutes(severity, affects = []) {
  let base = 0;

  if (severity === "critical") base = 45;
  else if (severity === "high") base = 30;
  else if (severity === "medium") base = 15;
  else if (severity === "low") base = 5;

  if (affects.includes("airport_security")) base += 10;
  if (affects.includes("airport_access")) base += 10;
  if (affects.includes("public_transport")) base += 10;
  if (affects.includes("road_access")) base += 5;
...
function buildFallbackSignals({ airport, origin, mode }) {
  ...
  if (transportMode === "public") {
    signals.push({
      ...
      limitations: [
        "Sinal baseado em perfil interno, ainda sem confirmação live.",
      ],
    });
  }
  ...
  if (airportProfile) {
    signals.push({
      ...
      limitations: [
        "Sinal preventivo; ainda não representa evento confirmado.",
      ],
    });
  }
