...
function normalizeSignal(signal) {
  const severityScore =
    ...
    severityToScore(signal.severity);
...
function calculateAggregate(signals) {
  ...
  if (operationalRiskScore >= 68) {
    operationalRisk = "high";
  } else if (operationalRiskScore >= 40) {
    operationalRisk = "medium";
  }

  let trustLevel = "low";

  if (confidenceScore >= 75 && liveSignalCount > 0) {
    trustLevel = "high";
...
export default async function handler(req, res) {
  try {
    const rawSignals = req.body?.signals || [];

    const normalizedSignals = rawSignals.map(normalizeSignal);
    const conflicts = detectConflicts(normalizedSignals);
    const aggregate = calculateAggregate(normalizedSignals);
    const recommendations = buildRecommendations(aggregate, conflicts);
    ...
    return res.status(200).json({
      ...
      metadata: {
        ...
        liveSignalCount: aggregate.liveSignalCount,
        highImpactSignalCount: aggregate.highImpactSignalCount,
      },

      limitations: [
        "Primeira versão de arbitragem heurística.",
