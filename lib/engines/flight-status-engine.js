...
const ENGINE_VERSION = "0.7.0-flight-data-gate";
...
export async function getFlightStatusIntelligence({
  ...
  const normalizedFlightNumber = String(flightNumber || "AF1195")
    .replace(/\s+/g, "")
    .toUpperCase();
  ...
  const requestedTerminal = terminal ? String(terminal) : null;

  const normalizedFlightDate = normalizeFlightDate(
    flightDate || manualDepartureTime || departureTime
  );

  const normalizedManualDepartureTime = forceManualTime
    ? manualDepartureTime || departureTime || null
    : null;

  const [aviationStackSource, aeroDataBoxSource] = await Promise.all([
    fetchAviationStackSource({
      flightNumber: normalizedFlightNumber,
      flightDate: normalizedFlightDate,
      airline: normalizedAirline,
      airport: normalizedAirport,
    }),
    fetchAeroDataBoxSource({
      flightNumber: normalizedFlightNumber,
      flightDate: normalizedFlightDate,
      airline: normalizedAirline,
      airport: normalizedAirport,
  ...
  return {
    ...
    provider: {
      name: success
        ? flightSourceDecision?.flightDecision?.selectedProvider ||
          "multi_source"
    ...
    flightDecision: flightSourceDecision?.flightDecision || null,
    sourceArbitration: flightSourceDecision,
    dataGate: flightSourceDecision?.dataGate || null,

    requiresUserConfirmation:
      flightSourceDecision?.flightDecision?.requiresUserConfirmation === true,

    reliability,
    operationalSignals,
    intelligenceSummary,

    sourceTransparency: buildSourceTransparency({
      decision: flightSourceDecision?.flightDecision,
      sources: usableSources,
      forceManualTime,
      requestedTerminal,
    ...
    diagnostics: {
      ...
      requestedFlightDate: normalizedFlightDate,
      ...
      aviationStack: {
        ...
        endpointUsed: aviationStackSource?.endpointUsed || null,
      ...
      flightDataGateActive: true,
      requiresUserConfirmation:
        flightSourceDecision?.flightDecision?.requiresUserConfirmation === true,
      hasLiveFlightData: flightSourceDecision?.dataGate?.hasLiveFlightData === true,

      trustFirstRulesActive: true,
      terminalGateInvented: false,
      requestedTerminalUsedAsLiveData: false,
...
async function fetchAviationStackSource({
  flightNumber,
  flightDate,
  airline,
  airport,
  ...
  if (airport) params.set("dep_iata", airport);
  if (airline) params.set("airline_iata", airline);

  const providerUrl = `http://api.aviationstack.com/v1/flights?${params.toString()}`;
  ...
  try {
    ...
    if (providerData?.error) {
      return {
        ...
        success: false,
        providerStatus,
        endpointUsed: sanitizeUrl(providerUrl),
        reason:
    ...
    const flightData = extractAviationStackFlight({
      providerData,
      flightNumber,
      airport,
      flightDate,
    ...
    const normalized = normalizeAviationStackFlightData(
      flightNumber,
      flightData
    );
  ...
  try {
    ...
    if (!providerResponse.ok) {
      return {
        ...
        reason: `AeroDataBox returned HTTP ${providerStatus}.`,
        providerPreview: providerData,
        source: null,
      };
    }

    const flightData = extractAeroDataBoxFlight(
      providerData,
      flightNumber,
      airport
    );
...
function buildProviderSource({
  ...
  return {
    ...
    estimatedDeparture:
      normalized.departure?.estimated || normalized.departure?.scheduled || null,
...
function extractAviationStackFlight({
  providerData,
  flightNumber,
  airport,
  flightDate,
  ...
  const normalizedDate = String(flightDate || "");

  return (
    candidates.find((item) => {
      const itemNumber = String(item?.flight?.iata || "")
        .replace(/\s+/g, "")
        .toUpperCase();

      const departureAirport = String(item?.departure?.iata || "").toUpperCase();
      const itemDate = String(item?.flight_date || "");
      ...
      if (
        normalizedAirport &&
        departureAirport &&
        departureAirport !== normalizedAirport
      ) {
        return false;
      }

      if (normalizedDate && itemDate && itemDate !== normalizedDate) {
        return false;
      }
...
function normalizeAeroDataBoxFlightData({
  ...
  const actualDepartureCandidate =
  ...
  const actualDeparture = isPastOperationalTime(actualDepartureCandidate)
    ? actualDepartureCandidate
    : null;

  const actualArrival = isPastOperationalTime(actualArrivalCandidate)
    ? actualArrivalCandidate
    : null;
...
function normalizeAviationStackFlightData(flightNumber, flightData) {
  ...
  const actualDeparture = isPastOperationalTime(flightData?.departure?.actual)
  ...
  const actualArrival = isPastOperationalTime(flightData?.arrival?.actual)
...
function buildSelectedFlight({
  ...
  const selectedSourceType = decision?.selectedSourceType;
  ...
  const trustedTerminal = isManualFallback
    ? null
    : decision.terminal || providerFlight?.departure?.terminal || null;

  const trustedGate = isManualFallback
    ? null
    : decision.gate || providerFlight?.departure?.gate || null;
  ...
  return {
    ...
    departure: {
      ...
      gate: trustedGate,
      terminalSource: trustedTerminal
        ? selectedProvider || "provider"
        : requestedTerminal
        ? "user_requested_not_confirmed"
        : "unavailable",
      gateSource: trustedGate ? selectedProvider || "provider" : "unavailable",
    },
...
function buildSourceTransparency({
  decision,
  sources,
  forceManualTime,
  requestedTerminal,
}) {
  const selectedSourceType = decision?.selectedSourceType || "unavailable";
  ...
  return {
    ...
    terminalPolicy:
      "Terminal supplied by user is treated as requested context, not confirmed live flight data.",
    gatePolicy:
      "Gate is only shown when returned by a trusted external flight provider.",
    maxConfidenceRule:
      selectedClassification === "manual"
        ? "Manual fallback must remain low confidence and require explicit user confirmation."
        : selectedClassification === "live"
        ? "Live provider data can receive higher confidence after arbitration."
        : "Estimated/fallback data should not appear as confirmed live data.",
    sources:
      sources?.map((source) => ({
        id: source.id,
        provider: source.provider,
        sourceType: source.sourceType,
        classification: source.sourceClassification || "live",
        liveDataActive: Boolean(source.liveDataActive),
...
function deriveOperationalStatus({
  ...
  if (raw.includes("arriv") || raw.includes("land")) {
    const arrivalCandidate = estimatedArrival || scheduledArrival;
    if (!arrivalCandidate || isPastOperationalTime(arrivalCandidate)) {
      return "landed";
    }
  }

  if (raw.includes("depart")) {
    const departureCandidate = estimatedDeparture || scheduledDeparture;
    if (!departureCandidate || isPastOperationalTime(departureCandidate)) {
      return "departed";
    }
  }
...
function isPastOperationalTime(value) {
...
function calculateProviderReliability(flight, sourceType) {
  ...
  if (!flight.departure?.scheduled && !flight.departure?.estimated) score -= 18;
  if (!flight.departure?.estimated) score -= 8;
  if (!flight.departure?.terminal) score -= 6;
  if (!flight.departure?.gate) score -= 6;

  if (!flight.route?.from?.code) score -= 8;
  if (!flight.route?.to?.code) score -= 8;
...
function calculateReliabilityFromDecision({ decision, sources }) {
  const selectedSourceType = decision?.selectedSourceType || "unavailable";
  const rawScore = decision?.confidenceScore || 0;

  const score =
    selectedSourceType === "manual_fallback_time"
      ? Math.min(rawScore, 18)
      : decision?.liveDataActive
      ? rawScore
...
function getLimitations(flight) {
  ...
  if (!flight.route?.to?.code) {
    limitations.push("Destino ainda não confirmado pela fonte de voo.");
  }
...
function buildOperationalSignals(flight, sourceDecision) {
  const signals = [];

  const requiresUserConfirmation =
    sourceDecision?.flightDecision?.requiresUserConfirmation === true;

  if (requiresUserConfirmation) {
    signals.push({
      type: "flight_requires_confirmation",
      label: "Voo sem validação live — requer confirmação",
      severity: "high",
    });
  }
  ...
  if (!flight.route?.to?.code) {
    signals.push({
      ...
      label: "Destino ainda não confirmado pela fonte de voo",
      severity: "medium",
    });
  }
...
function buildIntelligenceSummary(flight, sourceDecision) {
  ...
  if (requiresUserConfirmation) {
    return {
      operationalStatus: "confirmation_required",
      flightRisk: "high",
      recommendationImpact: "block_or_confirm_timeline",
      summary: `${flight.number} (${route}) não tem validação live suficiente. A Home2Flight deve bloquear a timeline automática ou pedir confirmação explícita antes de continuar.`,
    };
  }
...
function sanitizeUrl(url) {
  try {
    ...
    if (parsed.searchParams.has("access_key")) {
      parsed.searchParams.set("access_key", "hidden");
    }
