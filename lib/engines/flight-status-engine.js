...
const ENGINE_VERSION = "0.5.1-date-first-status-safe";
...
export async function getFlightStatusIntelligence({
  ...
  const normalizedManualDepartureTime =
    forceManualTime ? manualDepartureTime || departureTime || null : null;
...
function buildProviderSource({ id, provider, sourceType, normalized, reliability }) {
  return {
    id,
    provider,
    sourceType,
