...
export async function getFlightStatusIntelligence({
  ...
  try {
    ...
    const flightSourceDecision = await runFlightSourceArbitration({
      flight: normalized.number,
      airline:
        normalized.airline?.code ||
        normalizedAirline ||
        normalizedFlightNumber.slice(0, 2),
      airport: normalized.route?.from?.code || normalizedAirport,
      terminal: normalized.departure?.terminal || normalizedTerminal,
      manualDepartureTime: normalizedManualDepartureTime,
      forceManualTime,
      sources: [
        {
    ...
    return {
      ...
      version: "0.4.1-source-arbitrated-context-aware",
      provider: {
        name: "AviationStack",
        reachable: true,
...
async function buildFallbackResponse({
  ...
  const flightSourceDecision = await runFlightSourceArbitration({
    flight: flightNumber,
    airline: airline || flightNumber.slice(0, 2),
    airport: airport || "",
    ...
    sources: [
      {
        ...
        limitations: [
          ...
          "A fonte AviationStack não devolveu uma hora de partida utilizável.",
        ],
      },
    ],
  });
  ...
  return {
    ...
    diagnostics: {
      ...
      fallbackContextReceived: {
        ...
        manualDepartureTime: manualDepartureTime || null,
      },
    },
  };
}
