...
function getLimitations(flight) {
  ...
  if (!flight.departure.estimated) limitations.push("Hora estimada de partida ainda não disponível.");
  if (!flight.departure.terminal) limitations.push("Terminal de partida ainda não disponível.");
  if (!flight.departure.gate) limitations.push("Gate de partida ainda não disponível.");
  if (!flight.arrival.estimated) limitations.push("Hora estimada de chegada ainda não disponível.");
...
function buildOperationalSignals(flight) {
  ...
  if (flight.status === "cancelled") {
    signals.push({ type: "flight_cancelled", label: "Voo cancelado", severity: "high" });
  }

  if (flight.status === "landed" || flight.status === "likely_landed") {
    signals.push({ type: "flight_finished", label: "Voo já concluído", severity: "low" });
  }

  if (flight.status === "departed") {
    signals.push({ type: "flight_departed", label: "Voo já partiu", severity: "medium" });
  }

  if (flight.status === "delayed") {
    signals.push({ type: "flight_delayed", label: "Voo com atraso", severity: "medium" });
  }

  if (typeof flight.departure.delayMinutes === "number" && flight.departure.delayMinutes >= 30) {
    signals.push({
  ...
  if (!flight.departure.gate && !["landed", "likely_landed"].includes(flight.status)) {
    signals.push({ type: "gate_pending", label: "Gate ainda por confirmar", severity: "low" });
  }

  if (!flight.departure.terminal) {
    signals.push({ type: "terminal_pending", label: "Terminal ainda por confirmar", severity: "medium" });
  }

  if (signals.length === 0) {
    signals.push({ type: "flight_monitoring", label: "Voo em monitorização", severity: "low" });
  }
...
function buildIntelligenceSummary(flight) {
  ...
  if (
    ...
    (typeof flight.departure.delayMinutes === "number" && flight.departure.delayMinutes > 0)
  ) {
    return {
