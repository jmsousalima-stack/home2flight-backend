...
const ENGINE_VERSION = "1.9.6-date-first-flight-state-guard-lib";
...
function getFlightOperationalStatus(flightEngine) {
  return (
...
function buildFlightStateGuardResponse({ flightEngine, flightOperationalStatus }) {
...
export async function runJourneyPlanningEngine({
  ...
  const flightEngine = await getFlightStatusIntelligence({
  ...
  const flightOperationalStatus = getFlightOperationalStatus(flightEngine);

  const guardResponse = buildFlightStateGuardResponse({
    flightEngine,
    flightOperationalStatus,
  });
    ...
    flightEngine?.flight?.departure
      ? {
          ...
          providerStatusRaw:
            ...
            flightEngine?.flightDecision?.status ||
            "fallback",
          route: flightEngine?.flight?.route || {
            from: {
              code: normalizedAirport,
              icao: null,
              name: null,
              timezone: null,
            },
  ...
  return {
    ...
    diagnostics: {
      ...
      dateFirstMode: Boolean(normalizedFlightDate),
