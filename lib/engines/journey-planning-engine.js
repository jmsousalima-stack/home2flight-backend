...
function buildCompactJourneySummary(fullResponse) {
  const readiness = fullResponse.reliability?.readiness;
  const manualConfirmed = fullResponse.manualFlightConfirmed === true;
  const usedLiveFlightData = fullResponse.diagnostics?.usedLiveFlightData === true;

  if (
    readiness === "manual_confirmed_sensitive" ||
    (manualConfirmed && !usedLiveFlightData)
  ) {
    return {
      headline: "Timeline sensível: voo confirmado manualmente",
      explanation:
        "A timeline foi gerada, mas a hora do voo não foi validada por dados live.",
      mainRisk: "Sem validação live do voo",
      recommendedAction:
        "Confirmar novamente o estado do voo mais perto da partida.",
    };
  }

  if (fullResponse.reliability?.trustLevel === "high") {
    return {
      headline: "Timeline com confiança elevada",
      explanation:
        "A recomendação é suportada por dados operacionais consistentes.",
      mainRisk: null,
      recommendedAction: "Seguir a timeline recomendada.",
    };
  }

  return {
    headline: "Timeline operacional com margem prudente",
    explanation:
      "A recomendação combina dados disponíveis, perfil do aeroporto, rota, passageiros e buffers operacionais.",
    mainRisk: "Variabilidade operacional",
    recommendedAction:
      "Monitorizar o voo e condições do aeroporto antes da saída.",
  };
}

function buildCompactJourneyWarnings(fullResponse) {
  const warnings = [];

  if (
    fullResponse.manualFlightConfirmed === true &&
    fullResponse.diagnostics?.usedLiveFlightData !== true
  ) {
    warnings.push({
      type: "manual_flight_time",
      severity: "medium",
      message:
        "Hora do voo confirmada manualmente. Sem validação live de atraso, terminal, gate ou cancelamento.",
    });
  }

  if (!fullResponse.journey?.terminal) {
    warnings.push({
      type: "terminal_unconfirmed",
      severity: "medium",
      message: "Terminal ainda não confirmado por fonte externa de voo.",
    });
  }

  if (!fullResponse.journey?.gate) {
    warnings.push({
      type: "gate_unconfirmed",
      severity: "low",
      message: "Gate ainda não disponível por fonte externa de voo.",
    });
  }
...
function buildCompactSourcesSummary(fullResponse) {
  return {
    flight: {
      sourceType: fullResponse.sources?.flight?.sourceType || null,
      liveDataActive: fullResponse.sources?.flight?.liveDataActive === true,
      trustLevel:
        fullResponse.flightIntelligence?.flightDecision?.trustLevel ||
        fullResponse.sources?.flight?.decision?.trustLevel ||
        null,
    },
    airport: {
      sourceType: fullResponse.sources?.airport?.sourceType || null,
      liveDataActive: fullResponse.sources?.airport?.liveDataActive === true,
      trustLevel:
        fullResponse.sources?.airport?.confidenceLevel ||
        fullResponse.airportIntelligence?.operationalIntelligence
          ?.confidenceLevel ||
        null,
    },
    route: {
      sourceType: fullResponse.sources?.route?.sourceType || null,
      liveDataActive: fullResponse.sources?.route?.liveDataActive === true,
      trustLevel: fullResponse.sources?.route?.trustLevel || null,
    },
    weather: {
      sourceType: fullResponse.sources?.weather?.sourceType || null,
      liveDataActive: fullResponse.sources?.weather?.liveDataActive === true,
      trustLevel: fullResponse.sources?.weather?.trustLevel || null,
    },
  };
}

function buildCompactJourneyResponse(fullResponse) {
  return {
    success: fullResponse.success,
    engine: fullResponse.engine,
    version: fullResponse.version,
    generatedAt: fullResponse.generatedAt,
    journeyStatus: fullResponse.journeyStatus,
    manualFlightConfirmed: fullResponse.manualFlightConfirmed,
    journey: fullResponse.journey,
    decision: fullResponse.decision,
    operationalFlow: fullResponse.operationalFlow,
    buffers: fullResponse.buffers,
    reliability: {
      score: fullResponse.reliability?.score ?? null,
      trustLevel: fullResponse.reliability?.trustLevel ?? "low",
      readiness: fullResponse.reliability?.readiness ?? "unknown",
    },
    timeline: fullResponse.timeline,
    summary: buildCompactJourneySummary(fullResponse),
    warnings: buildCompactJourneyWarnings(fullResponse),
    sourcesSummary: buildCompactSourcesSummary(fullResponse),
  };
}
...
function buildReliabilityFromEngines({
  ...
  if (!usedLiveFlightData && manualFlightConfirmed) {
  ...
  return {
    ...
    readiness:
      !usedLiveFlightData && manualFlightConfirmed
        ? "manual_confirmed_sensitive"
        : score >= 75
        ? "ready"
        : score >= 50
        ? "sensitive"
...
export async function runJourneyPlanningEngine({
  ...
  const parsedDebug = parseBoolean(debug, false);
  ...
  if (["landed", "likely_landed", "departed"].includes(flightOperationalStatus)) {
    ...
    return parsedDebug ? fullResponse : buildCompactJourneyResponse(fullResponse);
  }
...
export function parseJourneyPlanningRequest(query = {}) {
  ...
  return {
    ...
    debug: parseBoolean(query.debug, false),
  };
}
