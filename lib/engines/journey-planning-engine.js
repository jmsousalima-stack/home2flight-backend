...
function getFlightDepartureTime({ flightEngine, manualDepartureTime }) {
  ...
  const decisionDeparture =
    flightDecision?.estimatedDeparture ||
    flightDecision?.scheduledDeparture;

  if (decisionDeparture && flightDecision?.sourceType !== "manual_fallback_time") {
    return {
      departureTime: decisionDeparture,
      source: flightDecision?.selectedSourceType || "flight_source_arbitration",
  ...
  if (decisionDeparture) {
    return {
      departureTime: decisionDeparture,
      source: flightDecision?.selectedSourceType || "manual_fallback_time",
      liveFlightUsed: Boolean(flightDecision?.liveDataActive),
    };
  }
...
function buildInternalOperationalSignals({
  ...
  if (departureResolution.liveFlightUsed) {
    ...
    signals.push({
      ...
      severity: "medium",
      confidenceScore:
        flightEngine?.flightDecision?.confidenceScore || 55,
...
export async function runJourneyPlanningEngine({
  ...
  const parsedBags = Boolean(bags);
  const parsedKids = Boolean(kids);
  const parsedCheckedIn = Boolean(checkedIn);
  const parsedFastTrack = Boolean(fastTrack);
  const parsedPriorityBoarding = Boolean(priorityBoarding);
  const normalizedFlightType = String(flightType || "passport");
  const parsedForceManualTime = Boolean(forceManualTime);
  ...
  if (!departureResolution.departureTime) {
    return {
      ...
      version: "1.9.3-flight-arbitration-aware-lib",
      error:
  ...
  const weatherSignals = buildWeatherOperationalSignals({
    weatherEngine,
  });
  ...
  return {
    ...
    timeline: [
      {
        ...
        status: "buffer",
        source:
          "Journey Planning Engine + Flight Source Arbitration + Weather Intelligence + Reliability Arbitration + Buffer Governance + Signal Priority",
      ...
      },
      {
        ...
        trustLevel: "medium",
        status: "buffer",
        source: "Airport Intelligence Engine + Weather Intelligence + Buffer Governance",
        liveInsight:
    ...
    diagnostics: {
      ...
      flightSourcesUsed:
        flightEngine?.flightDecision?.sourcesUsed || 0,
...
export function parseJourneyPlanningRequest(query = {}) {
  ...
  const transport = String(query.transport || query.mode || "public").toLowerCase();
