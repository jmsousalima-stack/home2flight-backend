...
export async function runJourneyPlanningEngine({
  ...
  if (!departureResolution.departureTime) {
    return {
      ...
      version: "1.9.0-lib-orchestrated",
      error:
  ...
  const airportEngine = await getAirportOperationalIntelligence({
    airport: normalizedAirport,
    ...
    timeline: [
      {
        ...
        status: "buffer",
        source:
          "Journey Planning Engine + direct /lib engines + Reliability Arbitration + Buffer Governance + Signal Priority",
