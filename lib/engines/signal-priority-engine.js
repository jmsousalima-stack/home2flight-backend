// /lib/engines/signal-priority-engine.js
...
function isPositiveSignal(signal) {
  ...
  const title = String(signal.title || signal.label || "").toLowerCase();
...
function normalizeSignal(signal) {
  ...
  if (!positive) {
    ...
    if (priorityScore >= 75) priority = "confidence_high";
    else if (priorityScore >= 55) priority = "confidence_medium";
    else priority = "confidence_low";
  }
...
function classifySignals(signals) {
  ...
  return {
    dominantRiskSignals: sortedRisk.filter(
      (signal) => signal.priority === "critical" || signal.priority === "high"
    ),
    supportingRiskSignals: sortedRisk.filter(
      (signal) => signal.priority === "medium"
    ),
    lowImpactRiskSignals: sortedRisk.filter(
      (signal) => signal.priority === "low"
    ),
    confidenceSupportSignals: sortedPositive.filter(
      (signal) =>
        signal.priority === "confidence_high" ||
        signal.priority === "confidence_medium"
    ),
    ignoredSignals: sortedRisk.filter(
      (signal) => signal.priority === "low" &&
...
export default runSignalPriorityEngine;