...
const AIRPORT_PROFILES = {
  LIS: {
    ...
    walkingMinutes: { min: 8, typical: 14, max: 22 },
    terminalProfiles: {
      "1": {
        terminal: "1",
        pressure: "medium",
  ...
  CDG: {
    ...
    walkingMinutes: { min: 12, typical: 22, max: 35 },
    terminalProfiles: {},
  },
  ...
  AMS: {
    ...
    walkingMinutes: { min: 12, typical: 22, max: 36 },
    terminalProfiles: {
      "1": {
        terminal: "1",
        pressure: "high",
  ...
  return {
    ...
    flags:
      terminalProfile.pressure !== "low"
        ...
        : [],
    limitations: ["Perfil de terminal ainda baseado em heurística interna."],
  };
}
...
function buildAirlineOperationLayer({ airline, baggageProfile }) {
  ...
  return {
    ...
    recommendedExtraBuffer: profile.recommendedExtraBuffer + (hasCheckedBag ? 8 : 0),
    confidenceScore: 58,
    confidenceLevel: "medium",
...
function buildSecurityIntelligenceLayer({
  ...
  const terminalRiskScore = RISK_SCORE[terminalIntelligence.terminalPressure] || 55;
  const variabilityScore = RISK_SCORE[terminalIntelligence.securityVariability] || 55;
